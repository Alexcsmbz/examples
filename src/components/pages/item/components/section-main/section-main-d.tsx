...
import {itemImage} from '../../page-item.styles';
...
export const SectionMainD = ({
  ...
  return (
    <CAdaptiveBox marginBottom="369px">
      <PBox display="grid" gap="10px">
        ...
        <PBox display="grid" gridTemplateColumns="auto 1fr" gap="26px" alignItems="start" justifyContent="start">
          ...
          <PBox display="grid" gap="24px">
            ...
            {mode === 'edit' && <FormEditItem />}
            {mode === 'put-on-sale' && <FormPutOnSale />}
            {seller !== '0' && seller?.toLowerCase() !== address?.toLowerCase() && (
              <PBox display="grid" justifyContent="start">
                <PButton variant="apply" onClick={onBuyClick}>
                  Buy item
                </PButton>
              </PBox>
            )}
