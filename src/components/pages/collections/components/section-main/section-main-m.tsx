...
import {PageCollectionsMProps} from '../../page-collections.types';

export const SectionMainM = (props: PageCollectionsMProps) => {
  return (
    <CAdaptiveBox className="fade-in" padding="16px 20px 40px">
      <PBox marginBottom="16px" display="grid" gridAutoFlow="column" justifyContent="space-between" alignItems="center">
        <PBox display="grid" gridAutoFlow="column" justifyContent="start" alignItems="baseline" gap="14px">
          <CBackButton inline>
            <Icon.ChevronLeft cursor="pointer" />
          </CBackButton>
          <PTypo variant="h2">{shortenAddress(props.selectedAuthor[0]?.name)} collections</PTypo>
        </PBox>
      ...
      <PBox minHeight="220px" margin="0 0 20px" position="relative">
        <PImage backgroundImage={props.authorProfile?.backgroundUrl!} backgroundColor={props.defaultBanner?.background}>
          <PBox display="grid" gap="19px">
            <PBox position="relative" justifySelf="center">
              <PAvatar
                ...
                mockImage={<Icon.Star fill="white" width={45} height={45} />}
            ...
            >
              <PTypo variant="h3" regular color={theme.pallete.light.common.white}>
      ...
      <PBox display="grid" gap="15px" marginBottom="24px">
        {props.authorProfile?.aboutMe && (
          <PBox display="grid" gap="5px">
            <PTypo bold>About me</PTypo>
            <PTypo>{props.authorProfile?.aboutMe}</PTypo>
          </PBox>
        )}

        <PBox display="grid" gap="11px" alignContent="start">
          <PBox display="grid" gridAutoFlow="column" justifyContent="start" gap="16px">
          ...
          {props.authorProfile?.website && (
            <PBox>
              <a
                ...
                href={
                  props.authorProfile?.website.includes('http://') || props.authorProfile?.website.includes('https://')
                    ? props.authorProfile?.website
      ...
      <PSidenav active={props.filterOpen}>
        ...
        >
          ...
          <PBox display="grid" gridTemplateColumns="1fr 1fr" gap="16px" justifyContent="center" alignItems="center">
            <PButton variant="apply" fullWidth disabled={!props.formIsDirty} onClick={props.onItemsFilter}>
              Apply
            </PButton>
